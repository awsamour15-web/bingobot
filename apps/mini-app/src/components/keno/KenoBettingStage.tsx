...
export function KenoBettingStage({
  ...
  const dec = () => {
    ...
    else if (betAmount <= 20) onChangeBet(Math.max(MIN_BET, betAmount - 2));
    else onChangeBet(Math.max(MIN_BET, betAmount - 10));
  };
  const inc = () => {
    if (betAmount < MIN_BET) onChangeBet(MIN_BET);
    else if (betAmount < 20) onChangeBet(Math.min(MAX_BET, betAmount + 2));
    else onChangeBet(Math.min(MAX_BET, betAmount + 10));
  };
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', userSelect: 'none' }}>
      <div style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(17,24,28,0.98)', border: `1px solid rgba(255,255,255,0.07)`, borderRadius: 14, padding: '10px 10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {spots === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, background: 'rgba(0,0,0,0.18)', borderRadius: 12, padding: '8px 8px 6px 15px', height: 129, boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, textAlign: 'center' }}>
              <div style={{ fontSize: 22, fontWeight: 900, color: '#edf5fb', letterSpacing: '-0.05em' }}>Choose 10 numbers</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#6ce7b0', letterSpacing: '-0.04em' }}>From 1 to 80</div>
            </div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, background: 'rgba(0,0,0,0.2)', borderRadius: 11, padding: '8px 8px 7px 15px', height: 129, boxSizing: 'border-box' }}>
            {/* Possible win row */}
            ...
            {/* Pay table row */}
            {payEntries.length > 0 && (
              <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 8, padding: '5px 8px', border: `1px solid rgba(255,255,255,0.06)`, lineHeight: 1.2 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 1fr)', gap: 3 }}>
              {Array.from({ length: 10 }).map((_, idx) => {
                ...
                return (
                  <div key={idx} style={{ height: 24, borderRadius: 5, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, fontFamily: 'monospace', background: num !== undefined ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.3)', border: `1px solid ${num !== undefined ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.05)'}`, color: num !== undefined ? C.textWhite : 'transparent' }}>
                    {num ?? '·'}
                  </div>
                );
              })}
        ...
        {/* 80-number grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, minmax(0, 1fr))', gap: 3, margin: '2px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <button onClick={dec} style={{ width: 40, height: 42, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: `1px solid ${C.border}`, color: C.textWhite, fontSize: 28, fontWeight: 300, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>−</button>
          <div style={{ flex: 1, height: 42, background: 'rgba(0,0,0,0.4)', border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 800, color: C.textWhite, fontFamily: 'monospace' }}>
            {betAmount % 1 === 0 ? betAmount : betAmount.toFixed(2)}
          </div>
          <button onClick={inc} style={{ width: 40, height: 42, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: `1px solid ${C.border}`, color: C.textWhite, fontSize: 26, fontWeight: 300, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>+</button>
          <button onClick={() => onChangeBet(Math.min(MAX_BET, Math.max(MIN_BET, betAmount * 2)))} style={{ padding: '0 12px', height: 42, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: `1px solid ${C.border}`, color: '#73f3b5', fontSize: 16, fontWeight: 800, cursor: 'pointer', flexShrink: 0 }}>X2</button>
          <button onClick={() => onChangeBet(Math.min(MAX_BET, Math.max(MIN_BET, Math.floor(userBalance))))} style={{ padding: '0 12px', height: 42, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: `1px solid ${C.border}`, color: '#73f3b5', fontSize: 16, fontWeight: 800, cursor: 'pointer', flexShrink: 0 }}>MAX</button>
